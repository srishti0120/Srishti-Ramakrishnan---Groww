...
export class AttentionService {
    static computeAttentionScore(symbol, alerts = {}) {
        const quote = db.prepare(`SELECT * FROM quote_cache WHERE symbol = ?`).get(symbol);
        if (!quote) {
            return {
                symbol,
                score: 0,
        ...
        const history = db.prepare(`SELECT close FROM daily_history WHERE symbol = ? ORDER BY date DESC LIMIT 2`).all(symbol);
