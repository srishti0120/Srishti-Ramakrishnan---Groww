...
export class SnapshotService {
    static captureSnapshot(userId, symbols) {
    ...
    `);
        ...
        db.transaction(() => {
            for (const symbol of symbols) {
                const quote = db.prepare(`SELECT price, volume FROM quote_cache WHERE symbol = ?`).get(symbol);
                if (quote) {
                    insert.run({
                        user_id: userId,
                        symbol,
    ...
    static getChangesSinceLastSeen(userId, currentQuotes) {
        const snapshots = db.prepare(`SELECT * FROM snapshots WHERE user_id = ?`).all(userId);
