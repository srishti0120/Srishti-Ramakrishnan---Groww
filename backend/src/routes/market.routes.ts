...
router.get('/search', (req, res) => {
  ...
  res.json(MarketDataService.searchSymbols(q as string));
});
...
router.get('/pulse/:userId', (req, res) => {
  ...
  const changeMap = new Map(changes.map((c: any) => [c.symbol, c]));

  let needsAttention = 0;
  let biggestMover: any = null;
  let maxChange = -1;
  ...
  resultItems.sort((a: any, b: any) => b.attention.score - a.attention.score);
