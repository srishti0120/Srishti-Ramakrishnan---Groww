...
router.get('/watchlists', (req, res) => {
  ...
  const watchlists = db.prepare('SELECT * FROM watchlists WHERE user_id = ? ORDER BY sort_order').all(userId) as any[];
  for (const wl of watchlists) {
    const countRes = db.prepare('SELECT COUNT(*) as c FROM watchlist_items WHERE watchlist_id = ?').get(wl.id) as any;
...
router.patch('/watchlists/:id/items/:symbol', (req, res) => {
  ...
  const updates = [];
  const params: any[] = [];
  if (alertAbove !== undefined) { updates.push('alert_price_above = ?'); params.push(alertAbove); }
  if (alertBelow !== undefined) { updates.push('alert_price_below = ?'); params.push(alertBelow); }
