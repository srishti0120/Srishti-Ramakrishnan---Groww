...
export class MarketDataService {
  static getStockMeta(symbol: string) {
    return MOCK_STOCKS.find(s => s.symbol === symbol.toUpperCase());
  }

  static generateHistoricalData(symbol: string, days = 30) {
  ...
  static getCurrentQuote(symbol: string) {
    const meta = this.getStockMeta(symbol);
    if (!meta) return null;

    // See if we have history
    let lastHistory = db.prepare(`SELECT * FROM daily_history WHERE symbol = ? ORDER BY date DESC LIMIT 1`).get(symbol) as any;
    if (!lastHistory) {
  ...
  static getQuote(symbol: string) {
    let cached = db.prepare(`SELECT * FROM quote_cache WHERE symbol = ?`).get(symbol) as any;
    if (!cached) {
  ...
  static getQuotes(symbols: string[]) {
    return symbols.map(s => this.getQuote(s)).filter(Boolean);
  }

  static searchSymbols(query: string) {
  ...
  static refreshAllQuotes() {
    const symbolsObj = db.prepare(`SELECT DISTINCT symbol FROM watchlist_items`).all() as { symbol: string }[];
    const symbols = symbolsObj.map(s => s.symbol);
    this.refreshQuotes(symbols);
  }

  static refreshQuotes(symbols: string[]) {
    const insert = db.prepare(`
